...
# imgElon = face_recognition.load_image_file("OngNoi/Check.jpg")
imgElon =cv2.cvtColor(imgElon,cv2.COLOR_BGR2RGB)
imgCheck = face_recognition.load_image_file("pic/elon check.jpg")
# imgCheck = face_recognition.load_image_file("OngNoi/OngNoi.jpg")
...
########
cv2.putText(imgCheck,f"{results}{(round(faceDis[0],2))}",(50,50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,255),3)
cv2.imshow("Elon",imgElon)  # view thử ảnh để kiểm tra
cv2.imshow("ElonCheck",imgCheck) # view thử ảnh


# cv2.imshow("OngNoi",imgElon)  # view thử ảnh để kiểm tra
# cv2.imshow("OngNoiCheck",imgCheck) # view thử ảnh
cv2.waitKey()
